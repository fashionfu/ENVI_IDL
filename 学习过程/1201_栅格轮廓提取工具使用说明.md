...
- **构建排除值列表**：
  ...
  **单/多波段分析逻辑详细流程**：
  ...
  ```mermaid
  flowchart TD
      Start[开始单/多波段分析] --> GetBands["获取波段数量<br/>numBands = Raster.NBANDS"]
      GetBands --> CheckBands{检查波段数量<br/>numBands == 1?}
      
      CheckBands -->|是| SingleBand[单波段处理路径]
      CheckBands -->|否| MultiBand[多波段处理路径]
      
      SingleBand --> SingleMethod["使用已知numRanges值<br/>（更可靠，避免SIZE解析问题）"]
      SingleMethod --> SingleSet["设置维度<br/>numRows = 2<br/>numCols = numRanges"]
      SingleSet --> SingleVerify{"验证数组访问<br/>excludeValues行0列numCols-1?"}
      
      SingleVerify -->|成功| SingleSuccess["单波段维度获取成功<br/>numRows x numCols"]
      SingleVerify -->|失败| SingleFallback["回退到SIZE方法<br/>sizeInfo = SIZE(excludeValues)"]
      SingleFallback --> SingleParse["解析SIZE结果<br/>numRows = sizeInfo索引2<br/>numCols = sizeInfo索引3"]
      SingleParse --> SingleSuccess
      
      MultiBand --> MultiCheck{检查numRanges<br/>是否可用?}
      MultiCheck -->|可用| MultiKnown["使用已知numRanges值<br/>（优先方法）"]
      MultiCheck -->|不可用| MultiSize["使用SIZE方法<br/>（备用方法）"]
      
      MultiKnown --> MultiSet["设置维度<br/>numRows = 2<br/>numCols = numRanges"]
      MultiSet --> MultiSuccess["多波段维度获取成功<br/>numRows x numCols"]
      
      MultiSize --> MultiGetSize["获取SIZE结果<br/>sizeInfo = SIZE(excludeValues)"]
      MultiGetSize --> MultiParse["解析SIZE结果<br/>格式: type, ndims, dim1, dim2, ...<br/>numRows = sizeInfo索引2<br/>numCols = sizeInfo索引3"]
      MultiParse --> MultiValidate{验证ndims >= 2?}
      MultiValidate -->|是| MultiSuccess
      MultiValidate -->|否| MultiError[错误: 无法确定维度]
      
      SingleSuccess --> Validate["验证数组维度<br/>确保 numRows >= 2<br/>numCols >= 1"]
      MultiSuccess --> Validate
      Validate --> End[维度获取完成<br/>准备创建掩膜]
      MultiError --> End
      
      style Start fill:#FFE4B5
      style End fill:#90EE90
      style SingleBand fill:#87CEEB
      style MultiBand fill:#DDA0DD
      style SingleMethod fill:#98FB98
      style MultiKnown fill:#98FB98
      style SingleFallback fill:#FFB6C1
      style MultiSize fill:#FFB6C1
      style SingleSuccess fill:#90EE90
      style MultiSuccess fill:#90EE90
      style MultiError fill:#FF6347
