...
```mermaid
flowchart TD
    A[输入文件 input_file<br/>Landsat 8/9 原始影像文件] --> B[GSF_GF1_RadiometricCorrection<br/>辐射定标：DN值 → 辐射亮度]
    ...
    D -->|OUTPUT_RASTER<br/>正射校正后影像| E[GSF_UrbanHeatIsland_LC8<br/>城市热岛分析]
    D -->|OUTPUT_RASTER<br/>正射校正后影像| F[ENVILandsat8LSTTask<br/>地表温度计算]
    E -->|OUTPUT_RASTER<br/>城市热岛结果| G[GSF_GetFileURL<br/>生成 out_options]
    F -->|OUTPUT_RASTER<br/>地表温度结果| G
    G --> H[out_options<br/>PngFileURL, MapExtent, ZipFileURL<br/>UrbanHeatIsland, LandSurfaceTemp]
    ...
    style H fill:#b3e5fc,stroke:#01579b,stroke-width:3px,color:#000
```
