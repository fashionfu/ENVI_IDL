...
;-
; $Id: mesh_obj.pro,v 1.4 1998/01/15 18:43:09 scottm Exp $
;
...
;
; PURPOSE:
;       rkmesh_obj generates a polygon mesh (vertex list and polygon list) that
...
;
; CALLING SEQUENCE:
;       rkmesh_obj, Type, Vertex_List, Polygon_List, Array1, Array2
;
; INPUTS:
;
...
;       rkmesh_obj, 3, Vertex_List, Polygon_List, Replicate(0.25, 48, 64), $
;          P4=0.5
;
...
;       rkmesh_obj, 6, Vertex_List, Polygon_List, $
...
;       Extensively modified by Ronn Kling June 1999.
;-

PRO rkmesh_obj, obj_type, vertex_list, polygon_list, array1, array2, $
    P1=p1, P2=p2, P3=p3, P4=p4, P5=p5, $
    Degrees=degrees, endcaps=endcaps, tex_coords=tex_coords
  ...
  CASE obj_type OF
    ...
    0: BEGIN ; Triangulated surface.
      ...
      ; next line adds in the number 3 for the polygon number
      polygon_list = [Replicate(3L, 1L, (N_Elements(tri) / 3L)), Temporary(tri)]
      ;a simple way to change a [m,n] array to an [m*n] array.
      polygon_list = polygon_list[*]
      if arg_present(tex_coords) then begin
        tex_coords = vertex_list[0:1,*] ;initialize array to right size
        ;replace with the correct values and set lowest value to zero
        tex_coords[1,*] = vertex_list[1,*] - min(vertex_list[1,*])
        tex_coords[0,*] = tex_coords[0,*] - (min(tex_coords[0,*]))
        tex_coords[0,*] = tex_coords[0,*]/(max(tex_coords[0,*])) ;normalize between 0 and 1
        tex_coords[1,*] = tex_coords[1,*]/(max(tex_coords[1,*])) ;normalize between 0 and 1
      endif
    ...
    2: BEGIN ; Polar surface.
      ...
      ENDIF else min_ang = 0.0
      
      IF (N_Elements(p2) GT 0L) THEN BEGIN
        max_ang = Float(p2)
        IF (Keyword_Set(degrees)) THEN max_ang = max_ang * !Dtor
      ENDIF else max_ang = 2.0 * !pi
      
      IF (N_Elements(p3) GT 0L) THEN min_rad = Float(p3) else  min_rad = 0.0
      
      IF (N_Elements(p4) GT 0L) THEN max_rad = Float(p4) else  max_rad = Float(dim_y - 1L)
      
      min_rad = min_rad > 1.0E-4
      max_rad = max_rad > 1.0E-4
      IF (min_ang EQ max_ang) THEN max_ang = max_ang + (2.0 * !PI)
      
      vertex_list[2, *] = Float(array1) ;z value
      vertex_list[1, *] = Replicate(1.0, dim_x) # $ ;radius values
        (min_rad + ((max_rad - min_rad) * Findgen(dim_y) / Float(dim_y - 1L)))
      vertex_list[0, *] = $ ;theta values
      ...
      if arg_present(tex_coords) then begin
        tex_coords = vertex_list[0:1,*] ;initialize array to right size
        tex_coords[1,*] = vertex_list[1,*] ;replace with the correct values
        tex_coords[0,*] = tex_coords[0,*]/(!Pi*2.0) ;normalize between 0 and 1
        tex_coords[1,*] = tex_coords[1,*]/(max(tex_coords[1,*])) ;normalize between 0 and 1
      endif
    ...
    3: BEGIN ; Cylindrical surface.
      ...
      IF (N_Elements(p3) GT 0L) THEN min_len = Float(p3) else min_len = 0.0
      
      IF (N_Elements(p4) GT 0L) THEN max_len = Float(p4) else max_len = Float(dim_y - 1L)
      
      IF (min_ang EQ max_ang) THEN max_ang = max_ang + (2.0 * !PI)
      
      vertex_list[1, *] = Float(array1) ;radius of cylinder
      vertex_list[2, *] = Replicate(1.0, dim_x) # $ ;z level of vertices
        (min_len + ((max_len - min_len) * Findgen(dim_y) / Float(dim_y - 1L)))
      vertex_list[0, *] = $ ;angle of vertices
        (min_ang + ((max_ang - min_ang) * Findgen(dim_x) / $
        Float(dim_x - 1L))) # Replicate(1.0, dim_y)
        
      indx_num = (dim_x - 1L) * (dim_y - 1L)
      poly_num = 5L * indx_num ;number of 4 sided polygons
      
      polygon_list = Lonarr(poly_num, /Nozero)
      y_inc = Replicate(1L, (dim_x - 1L)) # Lindgen(dim_y - 1L)
      
      p_ind = 5L * Lindgen(indx_num) ;index for every fourth point
      
      polygon_list[p_ind] = 4L ;number of points in polygon
      polygon_list[p_ind + 1L] = Lindgen(indx_num) + y_inc[*]
      y_inc = 0
      ...
      p_ind = 0
      if keyword_set(endcaps) then begin
        bottom = reverse(lindgen(dim_x)) ;bottom polygon
        top = lindgen(dim_x) + indx_num       ;top polygon
        ;tack the polygons on to the end of the list
        polygon_list = [polygon_list,dim_x,bottom,dim_x,top]
      endif
      if arg_present(tex_coords) then begin
        tex_coords = vertex_list[0:1,*] ;initialize array to right size
        tex_coords[1,*] = vertex_list[2,*] ;replace with the correct values
        tex_coords[0,*] = tex_coords[0,*]/(!Pi*2.0) ;normalize between 0 and 1
        tex_coords[1,*] = tex_coords[1,*]/(max(tex_coords[1,*])) ;normalize between 0 and 1
      endif
      vertex_list = CV_COORD(From_Cylin=vertex_list, /To_Rect)
    ...
    4: BEGIN ; Spherical surface.
      ...
      p_ind = 0
      if arg_present(tex_coords) then begin
        tex_coords = vertex_list[0:1,*]
        tex_coords[0,*] = tex_coords[0,*]/(!Pi*2.0) ;normalize between 0 and 1
        tex_coords[1,*] = (tex_coords[1,*]/(!Pi))+ 0.5 ;normalize between 0 and 1
      endif
    ...
    5: BEGIN ; Surface of extrusion.
      ...
      IF (N_Elements(p1) GT 0L) THEN steps = Long(p1) > 1L else steps = 1L
      ...
      IF (N_Elements(p2) GT 0L) THEN ex_vec = Float(p2) else ex_vec = [0.0, 0.0, 1.0]
      ...
      IF (y_len GT 0.0) THEN z_ang = Atan(ex_vec[1], ex_vec[0]) * !Radeg
      y_ang = 0.0
      IF ((ex_vec[2] NE 0.0) OR (y_len GT 0.0)) THEN y_ang = Atan(ex_vec[2], y_len) * !Radeg
      ...
      if arg_present(tex_coords) then begin
        tex_coords = vertex_list[0:1,*] ;initialize array to right size
        ;replace with the correct values and set lowest value to zero
        tex_coords[1,*] = vertex_list[2,*] - min(vertex_list[2,*])
        tex_coords[0,*] = tex_coords[0,*] - (min(tex_coords[0,*]))
        tex_coords[0,*] = tex_coords[0,*]/(max(tex_coords[0,*])) ;normalize between 0 and 1
        tex_coords[1,*] = tex_coords[1,*]/(max(tex_coords[1,*])) ;normalize between 0 and 1
      endif
    ...
    6: BEGIN ; Surface of revolution.
      sz_array = Size(array1)
      max_ind = sz_array[2] - 1L
      IF (N_Elements(p1) GT 0L) THEN steps = p1 > 3L else  steps = 3L
      ...
      IF (N_Elements(p2) GT 0L) THEN rot_point = Float(p2) else rot_point = [0.0, 0.0, 0.0]
      
      IF (N_Elements(p3) GT 0L) THEN rot_vec = Float(p3) else rot_vec = [0.0, 0.0, 1.0]
      ...
      IF (y_len GT 0.0) THEN z_ang = Atan(rot_vec[1], rot_vec[0]) * !Radeg
      y_ang = 0.0
      ...
      ;transform the points into the new frame
      IF (sz_array[1] EQ 2L) THEN $
        sweep = Vert_T3d([array1[0, *], array1[1, *], $
        Replicate(0.0, 1L, sz_array[2])]) $
      ELSE sweep = Vert_T3d(array1)
      
      ;make sure that no x values are near zero
      neg_ind = Where(sweep[0, *] LT 0.0,count)
      sweep[0, *] = Abs(sweep[0, *]) > 1.0E-4
      IF (count gt 0L) THEN sweep[0, neg_ind] = sweep[0, neg_ind] * (-1.0)
      neg_ind = 0
      ...
      if arg_present(tex_coords) then begin
        tex_coords = vertex_list[0:1,*] ;initialize array to right size
        ;replace with the correct values and set lowest value to zero
        tex_coords[0,*] = vertex_list[1,*] - min(vertex_list[1,*])
        tex_coords[0,*] = tex_coords[0,*]/(max(tex_coords[0,*])) ;normalize between 0 and 1
        tex_coords[1,*] = vertex_list[2,*] - min(vertex_list[2,*])
        tex_coords[1,*] = tex_coords[1,*]/(max(tex_coords[1,*])) ;normalize between 0 and 1
      endif
