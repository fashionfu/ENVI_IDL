...
;----------------------------------------------------------------------------
function SetMapParamet::Init, _REF_EXTRA=_extra
  ...
  self->SetMapParamet::_RegisterProperties
  self->_UpdateProjection
  
  if (N_ELEMENTS(_extra) gt 0) then $
    self->SetMapParamet::SetProperty, _EXTRA=_extra
...
;----------------------------------------------------------------------------
pro SetMapParamet::_RegisterProperties, $
...
;----------------------------------------------------------------------------
function SetMapParamet::_GetMapStructure
  ...
  self->SetMapParamet::GetProperty, ZONE=zone
...
;
pro SetMapParamet::_UpdateUTMLongitude
...
;
pro SetMapParamet::_UpdateStatePlane
...
;
pro SetMapParamet::_UpdateEllipsoid
...
;
pro SetMapParamet::_UpdateLimit
...
;----------------------------------------------------------------------------
pro SetMapParamet::_UpdateProjection
...
;-------------------------------------------------------------------------
function SetMapParamet::GetPreview
...
pro SetMapParamet::SetProperty, $
  ...
  if (N_ELEMENTS(strEllipsoid) eq 1) then begin
    ...
    if (match ge 0) then begin
      self->SetMapParamet::SetProperty, IELLIPSOID=match
    endif else begin
      self->ErrorMessage, SEVERITY=1, $
  ...
  if (N_ELEMENTS(semiMajor) eq 1 && self._semiMajor ne semiMajor) then begin
    ...
    self->SetMapParamet::GetProperty, IELLIPSOID=ell
    if (ell ne 0) then $
      self->SetMapParamet::SetProperty, IELLIPSOID=0
  endif
  ...
  if (N_ELEMENTS(gctpzone) eq 1) then begin
    ...
    if (self._projection eq 'State Plane') then begin
      ...
      if (ABS(gctpzone) le 60 && gctpzone ne 0) then begin
        ...
        self->SetMapParamet::SetProperty, UTM_ZONE=ABS(gctpzone)
      endif else begin
        badZone = 1
      endelse
    endelse
...
;
; PURPOSE:
;    Defines the object structure for an SetMapParamet object.
;
;-
pro  SetMapParamet__Define

  compile_opt idl2, hidden
  
  struct = { SetMapParamet, $
