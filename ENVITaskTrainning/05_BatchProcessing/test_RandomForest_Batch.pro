...
PRO test_RandomForest_Batch
  ...
  output_dir = FILEPATH('output', root_dir=ROUTINE_DIR())
  ...
  Abort = ENVIAbortable()
  Start = ENVIStartMessage('Machine Learning Classification', Abort)
  ...
  FOREACH element, files, i DO BEGIN

    ;错误处理，如果出现错误，记录错误文件和错误信息，继续下一个处理
    CATCH, errorStatus
    IF (errorStatus NE 0) THEN BEGIN
      CATCH, /CANCEL
      errMsgs = [errMsgs, element +' --- '+ !ERROR_STATE.MSG]
      MESSAGE, /RESET
      CONTINUE
    ENDIF
    ...
    infostr = STRTRIM(i+1,2)+'/'+STRTRIM(count,2)+': '+FILE_BASENAME(element)
    Progress = ENVIProgressMessage(infostr, percentProgress, Abort)
    Channel.Broadcast, Progress
    IF (Abort.ABORT_REQUESTED) THEN BREAK

    ;打开栅格
    input_raster = e.OpenRaster(element)

    ;设置输出路径
    fbasename = FILE_BASENAME(element, '.hdr')
    output_uri = FILEPATH(fbasename+'_rfc.dat', root_dir=output_dir)

    ;设置参数
    ClassifyTask.INPUT_RASTER = input_raster
    ClassifyTask.INPUT_MODEL = TrainTask.OUTPUT_MODEL
    ClassifyTask.OUTPUT_RASTER_URI = output_uri
    ClassifyTask.Execute
    e.Data.Add, ClassifyTask.OUTPUT_RASTER
