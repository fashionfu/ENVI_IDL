;一键发布 ESE 服务
;调用示例：
;IDL > SaveTask, 'C:/xxx/esetaskname.pro'
PRO SaveTask, pro_file,      $ ;输入pro文件完整路径
  same_dir    = same_dir,    $ ;设置此关键字，则发布到与pro文件同目录下
  copy_pro    = copy_pro,    $ ;设置此关键字，则同时拷贝pro源码文件
  no_copy_pro = no_copy_pro    ;此关键字不再起作用，默认不拷贝pro文件
  ...
  IF ~KEYWORD_SET(same_dir) THEN BEGIN
    ...
    search_flag = '/*.{task,style}'
    IF KEYWORD_SET(no_copy_pro) THEN search_flag = '/*.{task,style}'
    IF KEYWORD_SET(copy_pro)    THEN search_flag = '/*.{pro,task,style}'
