PRO GSF_Change_Detection,              $ ;变化检测服务
  ...
  feature_index    = feature_index,    $ ;可选。整型值。特征指数。可选项为 NDVI、NDWI、NDBI、NBR，默认为 NDVI。区分大小写
  ...
  export_vector    = export_vector,    $ ;可选。布尔型。是否输出矢量。可选项为0和1，默认为0
  ...
  IF KEYWORD_SET(export_vector) THEN BEGIN
    ...
    out_options += ORDEREDHASH("output_shapefile", output_shapefile, $
      "ZipShpURL", shp_options['ZipShpURL'])
  ENDIF
