PRO GSF_RPCOrthorectification,     $ ;常见卫星正射校正服务
  ...
  ;                                   "SPOT-6", "SPOT-7", "RapidEye", "Pleiades"
  ...
  pan_pixel_size = pan_pixel_size, $ ;可选。字符串。全色分辨率。若不设置，则根据传感器类型自行设置
  mss_pixel_size = mss_pixel_size, $ ;可选。字符串。多光谱分辨率。若不设置，则根据传感器类型自行设置
  resampling     = resampling,     $ ;可选。字符串。重采样方法。可选项有 Nearest Neighbor、Bilinear（默认项）、Cubic Convolution
  output_format  = output_format,  $ ;可选。字符串。输出格式。可选项有 ENVI、TIFF（默认项）
  output_path    = output_path,    $ ;可选。字符串。输出路径。若不设置，则输出到当前目录
  ...
  ;打开DEM文件
  IF ~KEYWORD_SET(dem_file) THEN dem_file = FILEPATH('GMTED2010.jp2', $
  ...
  CASE sensor_type OF
    'GF1-PMS'  : BEGIN  ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=2.0
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=8.0
      Bands=[2,1,0]
    END
    'GF1-WFV'  : BEGIN  ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.0
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=16.0
      Bands=[2,1,0]
    END
    'GF2'  : BEGIN      ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.8
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=3.2
      Bands=[2,1,0]
    END
    'GF4'  : BEGIN      ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=50
      Bands=[2,1,0]
    END
    'ZY3-01A'   : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=2.1
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=8.4
      Bands=[2,1,0]
    END
    'ZY3-02A'   : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=2.1
    ...
    END
    'ZY1-02C'   : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=5.0
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=10.0
      Bands=[2,1,0]
    END
    'BEIJING-2' : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=1.0
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=4.0
      Bands=[2,1,0]
    END
    'JL1-01A'   : BEGIN ;tif
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.72
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=2.88
      Bands=[2,1,0]
    END
    'SUPERVIEW-1': BEGIN;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.5
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=2.0
      Bands=[2,1,0]
    END
    'QUICKBIRD' : BEGIN ;til
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.61
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=2.44
      Bands=[2,1,0]
    END
    'SPOT-6'     : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=1.5
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=6.0
      Bands=[2,1,0]
    END
    'SPOT-7'     : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=1.5
    ...
    'WORLDVIEW-3': BEGIN ;til
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.31
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=1.24
      Bands=[2,1,0]
    END
    'WORLDVIEW-4': BEGIN ;til
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.31
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=1.14
      Bands=[2,1,0]
    END
    'RAPIDEYE'  : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.0
      IF ~ISA(mss_pixel_size) THEN mss_pixel_size=5.0
      Bands=[2,1,0]
    END
    'PLEIADES'  : BEGIN ;xml
      IF ~ISA(pan_pixel_size) THEN pan_pixel_size=0.5
  ...
  ;输出文件
  fbasename = GSF_FileBaseName(input_file) + '_RPCORTHO'
  ...
  CASE output_format OF
    ...
    'TIFF': fbasename += '.tif'
  ENDCASE
  output_file = FILEPATH(fbasename, root_dir=output_path)
  ...
  IF N_ELEMENTS(base_file) NE 0 && FILE_TEST(base_file) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      new_base_file = e.GetTemporaryFilename(/cleanup_on_exit)
      ;执行投影转换
  ...
  ;如果输出为TIFF
  IF output_format EQ 'TIFF' THEN Task.OUTPUT_RASTER.EXPORT, output_file, 'tiff'

  output_raster = e.OpenRaster(output_file)

  ;获取快视图/压缩包URL/地理范围
  GSF_GetFileURL, input_raster=output_raster, /get_pngfile_url, /get_map_extent, $
